/* ============================================================================
   OPX · NFT Platform — operator console (v2, ground-up rewrite)
   The control room for running the numismatic NFT marketplace. Everything an
   ops team needs to handle any issue: global search + Cmd-K, a health command
   center, users/dealers with full record detail + levers, orders & escrow with
   dispute resolution, marketplace moderation, a live chat inbox, finance,
   chain settlement, trust & safety, and an audit trail.

   All privileged reads/writes go through the keyless `admin` Supabase Edge
   Function (service role + HUB-JWT + ADMIN_EMAILS allowlist). No service key in
   the browser. Public catalog browsing still uses the publishable key.
   ========================================================================== */
(function () {
  const H = window.HELM;
  ...
  const copyable = (s) => s ? `<span class="opx-idcell"><span class="opx-id">${esc(String(s).slice(0, 10))}${String(s).length > 10 ? '…' : ''}</span><span class="opx-copy" data-copy="${esc(s)}" title="Copy">⧉</span></span>` : '—';
  const img = (url, cls) => url ? `<img class="opx-av ${cls || ''}" loading="lazy" src="${esc(url)}" alt="">` : `<div class="opx-av ${cls || ''}" style="display:grid;place-items:center;color:var(--t4)">◇</div>`;
  ...
  const pill = (text, tone) => `<span class="opx-pill ${tone || 'mut'}">${esc(text || '—')}</span>`;
  ...
  function sheet(title, metaHtml, bodyHtml, actsHtml) {
    ...
    ov.innerHTML = `<div class="opx-sheet"><div class="opx-sheet-h"><div style="min-width:0"><div class="ttl">${title}</div><div class="meta">${metaHtml || ''}</div></div><button class="opx-sheet-x" data-x>✕</button></div><div class="opx-sheet-b">${bodyHtml}</div>${actsHtml ? `<div class="opx-sheet-acts">${actsHtml}</div>` : ''}</div>`;
  ...
  const closeSheet = () => { const ov = document.querySelector('#opx-over'); if (ov) ov.classList.remove('open'); };
  ...
  /* ── navigation ── */
  const NAV = [
    { g: 'Command', items: [{ id: 'overview', l: 'Overview', ic: '◎' }] },
    { g: 'People', items: [{ id: 'users', l: 'Users', ic: '👤' }, { id: 'dealers', l: 'Dealers', ic: '🏷️' }] },
    { g: 'Commerce', items: [{ id: 'orders', l: 'Orders & escrow', ic: '📦' }, { id: 'listings', l: 'Listings', ic: '🏪' }, { id: 'sales', l: 'Sales', ic: '💶' }, { id: 'drops', l: 'Drops', ic: '🚀' }] },
    { g: 'Catalog', items: [{ id: 'collections', l: 'Collections', ic: '🗂️' }, { id: 'coins', l: 'Coins', ic: '🪙' }, { id: 'provision', l: 'Provisioning', ic: '🃏' }, { id: 'nfc', l: 'NFC cards', ic: '📡' }] },
    { g: 'Money', items: [{ id: 'finance', l: 'Finance', ic: '📊' }, { id: 'payouts', l: 'Payouts', ic: '⬇️' }, { id: 'royalties', l: 'Royalties', ic: '👑' }] },
    { g: 'Trust & ops', items: [{ id: 'inbox', l: 'Inbox', ic: '💬' }, { id: 'disputes', l: 'Disputes', ic: '⚖️' }, { id: 'counterfeit', l: 'Counterfeit', ic: '🛡️' }, { id: 'chain', l: 'Chain & gas', ic: '⛓️' }, { id: 'audit', l: 'Audit log', ic: '📜' }] },
  ];
  const META = {
    ...
    provision: ['Provisioning', 'Issue one-time-claimable NFC cards per coin.'],
    ...
    chain: ['Chain & gas', 'The Base settlement queue — mints and transfers, gas sponsored.'],
  ...
  function render(el) {
    root = el;
    const links = NAV.map((g) => `<div class="opx-grp">${esc(g.g)}</div>${g.items.map((t) => `<button class="opx-link${t.id === active ? ' on' : ''}" data-tab="${t.id}"><span class="ic">${t.ic}</span>${esc(t.l)}<span class="ct" data-badge="${t.id}" hidden></span></button>`).join('')}`).join('');
    el.innerHTML = `<div class="opx"><div class="opx-shell">
      <aside class="opx-rail" id="opx-rail">
        <div class="opx-brand"><div class="mk">🌉</div><div><b>NFT Platform</b><span>Operator console</span></div></div>
      ...
      <main class="opx-main">
        <div class="opx-top">
          <button class="opx-topbtn opx-burger" id="opx-burger">☰</button>
          <div class="opx-search">
            <span class="sx">⌕</span>
            <input id="opx-q" placeholder="Search users, dealers, coins, orders, certs, tx…" autocomplete="off"/>
            <span class="kbd">⌘K</span>
            <div class="opx-results" id="opx-res" style="display:none"></div>
          </div>
          <button class="opx-topbtn" id="opx-refresh" title="Refresh">⟳</button>
        </div>
  ...
  async function paint() {
    ...
    try {
      const fn = VIEWS[active]; if (fn) await fn(body); else body.innerHTML = `<div class="opx-empty">Coming soon.</div>`;
    } catch (e) { body.innerHTML = `<div class="opx-empty">Failed to load: ${esc(e.message)}</div>`; }
    wireCopy(body);
  }
  ...
  function setupSearch(el) {
    ...
    const run = debounce(async () => {
      ...
      box.innerHTML = [
        ...
        grp('Certificates', res.certificates, (c) => `<div class="opx-res" data-nav="coins"><span class="rt">${esc(c.serial)}</span><span class="rs">cert</span></div>`),
  ...
  function openCmd() {
    ...
    const cmds = ALL.map((t) => ({ label: t.l, ic: t.ic, run: () => go(t.id), hint: 'Go' }));
    ...
    function rebuild() {
      ...
      if (q.length >= 2) items.push({ label: `Search platform for “${inp.value.trim()}”`, ic: '⌕', hint: 'Search', run: () => { const s = root.querySelector('#opx-q'); s.value = inp.value.trim(); s.dispatchEvent(new Event('input')); s.focus(); } });
      sel = 0; draw();
    }
    function draw() { list.innerHTML = items.map((c, i) => `<div class="opx-cmd-i ${i === sel ? 'sel' : ''}" data-i="${i}"><span class="ic">${c.ic}</span>${esc(c.label)}<span class="hint">${esc(c.hint)}</span></div>`).join('') || `<div class="opx-cmd-i">No commands</div>`; list.querySelectorAll('[data-i]').forEach((b) => b.addEventListener('click', () => { items[+b.dataset.i].run(); close(); })); }
  ...
  VIEWS.overview = async (body) => {
    ...
    if (!s || !s.ok) { body.innerHTML = headHTML('overview') + `<div class="opx-empty">Sign in to load the command center.</div>`; return; }
    ...
    const attn = [
      { id: 'disputes', n: c.disputes, l: '⚖️ Disputes', cls: c.disputes ? 'hot' : 'calm' },
      { id: 'dealers', n: c.dealer_pending, l: '🏷️ Dealer applications', cls: c.dealer_pending ? 'warm' : 'calm' },
      { id: 'payouts', n: c.withdrawals, l: '⬇️ Payouts to release', cls: c.withdrawals ? 'warm' : 'calm' },
      { id: 'counterfeit', n: c.counterfeit, l: '🛡️ Counterfeit reports', cls: c.counterfeit ? 'hot' : 'calm' },
      { id: 'chain', n: c.chain_queued, l: '⛓️ Chain jobs queued', cls: c.chain_queued ? 'warm' : 'calm' },
    ...
    body.innerHTML = headHTML('overview') +
      ...
      `<div class="opx-panel"><div class="opx-panel-h"><h3>Needs you</h3><span class="sub">— resolve these to keep the platform healthy</span></div>
        <div class="opx-panel-b"><div class="opx-attn">${attn.map((a) => `<div class="opx-attn-c ${a.n ? a.cls : 'calm'}" data-go="${a.id}"><div class="n opx-num">${a.n || 0}</div><div class="lbl">${a.l}</div><div class="go">${a.n ? 'Open →' : 'All clear ✓'}</div></div>`).join('')}</div></div></div>
      <div class="opx-grid2">
        ...
        <div class="opx-panel"><div class="opx-panel-h"><h3>Platform at a glance</h3></div><div class="opx-panel-b flush">
          ${table([{ h: 'Entity', cell: (r) => r[0] }, { h: '', num: true, cell: (r) => `<b class="opx-num">${r[1]}</b>` }], [
            ['👤 Users', num(c.users)], ['🏷️ Dealers', num(c.dealers)], ['🗂️ Collections', num(c.collections)], ['🪙 Coins', num(c.coins)], ['🎫 Certificates', num(c.certificates)], ['🏪 Active listings', num(c.listings_active)], ['📦 Orders in flight', num(c.orders_open)],
          ])}
        </div></div>
      </div>`;
  ...
  VIEWS.users = async (body) => {
    ...
    const load = async (q) => {
      ...
      if (!r.ok) { wrap.innerHTML = `<div class="opx-empty">${esc(r.error || 'Could not load')}</div>`; return; }
      wrap.innerHTML = `<div class="opx-panel"><div class="opx-panel-b flush">` + table([
        ...
        { h: 'Status', cell: (u) => u.suspended ? pill('suspended', 'bad') : pill('active', 'ok') },
  ...
  async function userDetail(id) {
    ...
    if (!r.ok) { sheet('User', '', `<div class="opx-empty">${esc(r.error || 'not found')}</div>`); return; }
    const u = r.user;
    ...
    const body = `
      ...
      ${r.certs && r.certs.length ? `<div><div class="opx-sec-t">Owned certificates (${r.certs_count})</div>${table([{ h: 'Serial', cell: (c) => mono(c.serial) }, { h: 'Minted', cell: (c) => c.minted ? pill('on-chain', 'ok') : pill('off-chain', 'mut') }], r.certs.slice(0, 12))}</div>` : ''}`;
  ...
  VIEWS.dealers = async (body) => {
    const r = await get('/api/dealers'); if (!r.ok) { body.innerHTML = headHTML('dealers') + `<div class="opx-empty">${esc(r.error || 'Could not load')}</div>`; return; }
    ...
    body.innerHTML = headHTML('dealers') +
      ...
      `<div class="opx-panel"><div class="opx-panel-h"><h3>All dealers</h3></div><div class="opx-panel-b flush">` + table([
        ...
        { h: 'Royalty', num: true, cell: (d) => ((d.default_royalty_bps || 0) / 100) + '%' },
        { h: 'Status', cell: (d) => `${pill(d.status, TONE.dealer(d.status))}${d.verified ? ' ' + pill('verified', 'info') : ''}` },
  ...
  async function dealerDetail(id) {
    sheet('Loading…', '', loading());
    const r = await get('/api/dealers?id=' + encodeURIComponent(id)); if (!r.ok) { sheet('Dealer', '', `<div class="opx-empty">${esc(r.error || 'not found')}</div>`); return; }
    const d = r.dealer;
  ...
  VIEWS.orders = async (body) => {
    ...
    const r = await get('/api/orders' + (status ? '?status=' + status : '')); if (!r.ok) { body.innerHTML = headHTML('orders') + `<div class="opx-empty">${esc(r.error || 'Could not load')}</div>`; return; }
    const t = r.totals || {};
    ...
    body.innerHTML = headHTML('orders') +
      ...
      `<div class="opx-panel"><div class="opx-filters"><div class="opx-seg">${seg.map((s) => `<button class="${status === s ? 'on' : ''}" data-f="${s}">${s || 'All'}</button>`).join('')}</div></div><div class="opx-panel-b flush">` +
      table([
        ...
        { h: 'Status', cell: (o) => pill((o.status || '').replace(/_/g, ' '), TONE.order(o.status)) },
  ...
  async function orderDetail(id) {
    sheet('Loading…', '', loading());
    const r = await get('/api/orders?id=' + encodeURIComponent(id)); if (!r.ok) { sheet('Order', '', `<div class="opx-empty">${esc(r.error || 'not found')}</div>`); return; }
    const o = r.order;
  ...
  VIEWS.listings = async (body) => {
    ...
    const r = await get('/api/listings?status=' + status); if (!r.ok) { body.innerHTML = headHTML('listings') + `<div class="opx-empty">${esc(r.error || 'Could not load')}</div>`; return; }
    const seg = ['active', 'sold', 'cancelled'];
    body.innerHTML = headHTML('listings') +
      `<div class="opx-panel"><div class="opx-filters"><div class="opx-seg">${seg.map((s) => `<button class="${status === s ? 'on' : ''}" data-f="${s}">${s}</button>`).join('')}</div><span class="sub" style="color:var(--t3);font-size:12px">${(r.rows || []).length} listings</span></div><div class="opx-panel-b flush">` +
      table([
        ...
        { h: 'Status', cell: (l) => pill(l.status, TONE.listing(l.status)) },
  ...
  VIEWS.finance = async (body) => {
    const r = await get('/api/accounting'); if (!r.ok) { body.innerHTML = headHTML('finance') + `<div class="opx-empty">${esc(r.error || 'Could not load')}</div>`; return; }
  ...
  VIEWS.disputes = async (body) => {
    ...
    body.innerHTML = headHTML('disputes') + `<div class="opx-panel"><div class="opx-panel-h"><h3>Frozen trades</h3><span class="sub">${d.length} awaiting a ruling</span></div><div class="opx-panel-b flush">` + table([
    ...
    ], d, { click: true, empty: 'No open disputes ✓', rowAttr: (o) => `data-o="${esc(o.id)}"` }) + `</div></div>`;
  ...
  VIEWS.counterfeit = async (body) => {
    ...
    body.innerHTML = headHTML('counterfeit') + `<div class="opx-panel"><div class="opx-panel-b flush">` + table([
      ...
      { h: 'Status', cell: (x) => `<select class="opx-inp" data-report="${esc(x.id)}" style="height:30px">${opts.map((o) => `<option ${x.status === o ? 'selected' : ''}>${o}</option>`).join('')}</select>` },
    ], c, { empty: 'No open reports ✓' }) + `</div></div>`;
  ...
  VIEWS.chain = async (body) => {
    const r = await get('/api/console?view=chain'); if (!r.ok) { body.innerHTML = headHTML('chain') + `<div class="opx-empty">${esc(r.error || 'error')}</div>`; return; }
    const c = r.counts || {};
    body.innerHTML = headHTML('chain') +
      kpis([{ l: 'Gas tank (Base)', v: r.balance_eth != null ? r.balance_eth.toFixed(4) + ' Ξ' : '—', cls: 'accent' }, { l: 'Queued', v: num(c.queued || 0), cls: c.queued ? 'warn' : '' }, { l: 'Settled', v: num(c.done || 0), cls: 'ok' }, { l: 'Failed', v: num(c.failed || 0), cls: c.failed ? 'bad' : '' }]) +
      `<div class="opx-panel"><div class="opx-panel-h"><h3>Settlement queue</h3><span class="sub">drained automatically by cron</span></div><div class="opx-panel-b flush">` + table([
        { h: 'Type', cell: (j) => pill(j.type, 'mut') }, { h: 'To', cell: (j) => j.to_address ? `<span class="opx-id">${sid(j.to_address)}…</span>` : '—' }, { h: 'Status', cell: (j) => `${pill(j.status, TONE.chain(j.status))}${j.tx_hash ? ' ' + copyable(j.tx_hash) : ''}` }, { h: 'Tries', num: true, cell: (j) => j.attempts }, { h: 'Age', num: true, cell: (j) => `<span style="color:var(--t3)">${ago(j.created_at)}</span>` },
      ...
      ], r.jobs || [], { empty: 'No chain jobs.' }) + `</div></div>`;
    ...
    body.querySelectorAll('[data-done]').forEach((b) => b.addEventListener('click', async () => { const tx = prompt('Tx hash (0x…) — blank = mark failed:'); if (tx === null) return; const r2 = await post('console', { action: 'mark_chain_job', id: b.dataset.done, tx: tx.trim() || undefined, failed: !tx.trim() }); if (r2.ok) { toast('Updated ✓', 'ok'); paint(); } else toast('Failed', 'bad'); }));
  };
  ...
  VIEWS.inbox = async (body) => {
    ...
    body.innerHTML = headHTML('inbox') + `<div class="opx-inbox"><div class="opx-convs" id="opx-convs">${convs.length ? convs.map((c) => `<div class="opx-conv" data-c="${esc(c.id)}"><div class="cp">${c.dealer_id ? '🏷️' : '📦'} ${esc(c.party || 'conversation')}</div><div class="cm">${esc(c.kind || 'chat')}${c.order_id ? ' · order ' + sid(c.order_id) : ''} · ${ago(c.last_message_at)}</div></div>`).join('') : '<div class="opx-empty">No conversations.</div>'}</div><div class="opx-thread" id="opx-thread"><div class="opx-empty">Select a conversation to read & reply.</div></div></div>`;
  ...
  async function openThread(cid, host) {
    ...
    const load = async () => {
      ...
      msgsEl.innerHTML = msgs.length ? msgs.map((m) => `<div class="opx-msg ${m.sender_id && m.sender_id === myId ? 'me' : 'them'}">${esc(m.body || '')}<div class="mt">${dt(m.created_at)}</div></div>`).join('') : '<div class="opx-empty">No messages yet.</div>';
  ...
  VIEWS.provision = async (body) => {
    const r = await get('/api/provision'); if (!r.ok) { body.innerHTML = headHTML('provision') + `<div class="opx-empty">${esc(r.error || 'Could not load')}</div>`; return; }
    const rows = r.rows || [];
    const certBase = (d) => (d && d.custom_domain && d.custom_domain_verified) ? d.custom_domain : 'opulence-tech.vercel.app';
    body.innerHTML = headHTML('provision') + rows.map((c) => `
      ...
      </div><div data-out="${esc(c.id)}"></div></div></div>`).join('') || `<div class="opx-empty">No collections.</div>`;
    ...
    body.querySelectorAll('[data-prov]').forEach((b) => b.addEventListener('click', async () => {
      ...
      out.innerHTML = `<p style="color:var(--warn);font-size:12px;margin:12px 0 6px;font-weight:600">⚠ Claim codes shown once — download now.</p>` + table([{ h: 'Coin', cell: (c) => esc(c.coin_name || '') }, { h: 'UID', cell: (c) => mono(c.uid) }, { h: 'Claim code', cell: (c) => `<b style="color:var(--ac)" class="opx-mono">${esc(c.claim_code)}</b>` }], cards) + `<button class="opx-btn sm" id="csv-${id}" style="margin-top:10px">⬇ Download CSV</button>`;
  ...
  VIEWS.collections = async (body) => {
    ...
    body.innerHTML = headHTML('collections') + `<div class="opx-panel"><div class="opx-panel-b flush">` + table([
      ...
      { h: 'State', cell: (c) => `${c.approved ? pill('approved', 'ok') : pill('pending', 'warn')} ${c.published ? pill('published', 'info') : pill('draft', 'mut')}${c.featured ? ' ' + pill('★', 'info') : ''}` },
  ...
  VIEWS.nfc = async (body) => {
    ...
    body.innerHTML = headHTML('nfc') + kpis(STAT.map((s) => ({ l: s, v: num(by[s] || 0) })), 4) +
      `<div class="opx-panel"><div class="opx-filters"><input class="opx-inp" id="nfc-uid" placeholder="Register new tag UID…" style="width:280px"/><button class="opx-btn sm" id="nfc-reg">Register</button></div><div class="opx-panel-b flush">` + table([
        { h: 'UID', cell: (t) => mono(t.uid) }, { h: 'Status', cell: (t) => pill(t.status, TONE.nfc(t.status)) }, { h: 'Coin', cell: (t) => t.coin_id ? mono(sid(t.coin_id)) : '—' }, { h: 'Taps', num: true, cell: (t) => t.tap_count || 0 },
  ...
  VIEWS.drops = async (body) => {
    ...
    body.innerHTML = headHTML('drops') + `<div class="opx-panel"><div class="opx-panel-b flush">` + table([
      { h: 'Title', cell: (d) => `<span class="opx-strong">${esc(d.title)}</span>` }, { h: 'Launch', cell: (d) => when(d.launch_at) }, { h: 'Supply', num: true, cell: (d) => d.supply ?? '—' }, { h: 'Price', num: true, cell: (d) => eur(d.price_eur) }, { h: 'Status', cell: (d) => pill(d.status || '—', d.status === 'live' ? 'ok' : 'mut') },
    ], rows, { empty: 'No drops.' }) + `</div></div>`;
  };

  H.register({ id: 'nft-site', label: 'NFT Platform', icon: '🛰️', scope: 'company', render });
})();
