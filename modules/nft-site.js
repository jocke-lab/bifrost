...
(function () {
  ...
  ICON.programming = ICON.provision;
  ...
  async function api(path, opts) {
    ...
    try {
      ...
      if (mm && mm[1] === 'programming') { target = EDGE.replace('/admin/', '/admin-programming') + (mm[2] || ''); headers.apikey = PUBKEY; }
      else if (mm && ADMIN_RES.has(mm[1])) { target = EDGE + mm[1] + (mm[2] || ''); headers.apikey = PUBKEY; }
  ...
  const NAV = [
    ...
    { g: 'Catalog', items: [{ id: 'collections', l: 'Collections' }, { id: 'coins', l: 'Coins' }, { id: 'provision', l: 'Provisioning' }, { id: 'programming', l: 'Card programming' }, { id: 'nfc', l: 'NFC cards' }] },
  ...
  const META = {
    ...
    programming: ['Card programming', 'Dealer requests to program a collection’s cards. Start one to make it the active job, then tap the cards in the desktop Card Programmer.'],
  ...
  /* PROVISIONING */
  VIEWS.programming = async (body) => {
    const r = await get('/api/programming');
    if (!r.ok) { body.innerHTML = headHTML('programming') + emptyState('alert', 'Could not load', r.error); return; }
    const rows = r.rows || [], active = r.active || null;
    const open = rows.filter((x) => ['requested', 'queued'].includes(x.status));
    const st = (s) => s === 'in_progress' ? 'info' : s === 'done' ? 'ok' : s === 'cancelled' ? 'bad' : 'warn';
    body.innerHTML = headHTML('programming') +
      (active ? `<div class="opx-panel" style="border-color:var(--ac-line)"><div class="opx-panel-h"><h3>${ic('nfc')}Active job</h3><span class="sub">the desktop Card Programmer pulls this</span></div><div class="opx-panel-b"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><div class="opx-strong">${esc(active.dealer_name || '—')} — ${esc(active.collection_name || '—')}</div><div class="opx-id" style="margin-top:4px">${active.cards_done}/${active.coins} cards programmed · tap the rest in the desktop app</div></div><div class="opx-rowacts"><button class="opx-btn" data-done="${esc(active.id)}">Mark done</button><button class="opx-btn danger" data-cancel="${esc(active.id)}">Cancel</button></div></div></div></div>` : '') +
      `<div class="opx-panel"><div class="opx-panel-h"><h3>Requests</h3><span class="sub">${open.length} waiting</span></div><div class="opx-panel-b flush">` +
      table([
        { h: 'Company', cell: (x) => `<span class="opx-strong">${esc(x.dealer_name || '—')}</span>` },
        { h: 'Collection', cell: (x) => esc(x.collection_name || '—') },
        { h: 'Cards', num: true, cell: (x) => `${x.cards_done}/${x.coins}` },
        { h: 'Status', cell: (x) => pill((x.status || '').replace(/_/g, ' '), st(x.status), x.status === 'in_progress') },
        { h: 'Requested', num: true, cell: (x) => `<span style="color:var(--t3)">${ago(x.created_at)}</span>` },
        { h: '', num: true, cell: (x) => x.status === 'in_progress' ? `<button class="opx-mini" data-done="${esc(x.id)}">Done</button>` : (['requested', 'queued'].includes(x.status) ? `<button class="opx-mini" data-start="${esc(x.id)}">Start programming</button>` : `<span style="color:var(--t3)">${esc(x.status)}</span>`) },
      ], rows, { empty: 'No programming requests yet — dealers request from their collection page.' }) + `</div></div>`;
    const act = async (action, id, extra) => { const r2 = await post('programming', Object.assign({ action, id }, extra || {})); if (r2.ok) { toast('Updated', 'ok'); paint(); } else toast('Failed: ' + (r2.error || ''), 'bad'); };
    body.querySelectorAll('[data-start]').forEach((b) => b.addEventListener('click', () => act('start', b.dataset.start)));
    body.querySelectorAll('[data-done]').forEach((b) => b.addEventListener('click', () => act('set_status', b.dataset.done, { status: 'done' })));
    body.querySelectorAll('[data-cancel]').forEach((b) => b.addEventListener('click', () => act('set_status', b.dataset.cancel, { status: 'cancelled' })));
  };
